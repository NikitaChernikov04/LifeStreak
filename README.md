...
> **Миграции при деплое.** `vercel-build` прогоняет `scripts/apply-schema.mjs`
> перед сборкой, поэтому новый код никогда не выкатывается на старую схему.
> Скрипт идемпотентен: уже применённые миграции пропускаются.
